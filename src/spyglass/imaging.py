...
# Distinct from any grayscale data value, so the seams between tiles read
# unambiguously as "not weight data" rather than as extreme brightness.
DEFAULT_GAP_COLOR = (255, 64, 0)


def write_grayscale_png(arr_u8: np.ndarray, path: Path) -> None:
...
def compose_tile_grid(
    ...
    *,
    ...
    """Arranges a stack of grayscale tiles (n, rows, cols) into one RGB grid
    ...
    """
    n, tile_rows, tile_cols = tiles_u8.shape
    ...
    for idx in range(n):
        ...
        canvas[y0 : y0 + tile_rows, x0 : x0 + tile_cols] = tiles_u8[idx, :, :, None]
